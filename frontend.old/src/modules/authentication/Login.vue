  ...
  <v-card>
    ...
    <v-card-text>
      <v-form rounded v-model="isLoginFormValid">
        <v-text-field name="email" label="Email*" id="email" v-model="email" prepend-icon="mdi-account-circle" required
          :rules="[checkIsRequired(email), validateEmail(email)]" type="email"></v-text-field>

        <v-text-field name="password" label="Password*" id="password" v-model="password" :type="getPasswordFieldType()"
          prepend-icon="mdi-lock" :append-icon="getShowPasswordApendIcon()" @click:append="toggleShowPassword()"
    ...
    <v-card-actions>
      ...
      <v-spacer></v-spacer>
      <v-btn color="success" :disabled="!isLoginFormValid || isProcessing || GetIsSnackbarVisible"
        @click="login({ email: email, password: password })">Login</v-btn>
    </v-card-actions>
    <SnackNotification v-bind:visibility="GetIsSnackbarVisible" v-bind:colorCondition="getIsUserLoggedIn"
...
import SnackNotification from "../notification/SnackNotification.vue";
export default {
  name: "Login",
  data: function () {
    return {
